...
/* カーネルプロセス (PID 0) */
static struct process kernel_process = {.pid = 0,
                                        .ppid = -1,
                                        .name = "kernel",
...
/* シェルプロセス (PID 1) */
static struct process shell_process = {.pid = 1,
                                       .ppid = 0,
                                       .name = "shell",
                                       .state = PROCESS_STATE_RUNNING,
                                       .exit_code = 0};
 ...
 * 現在実行中のプロセスを取得
 */
struct process *process_get_current(void) { return g_process_table.current; }
